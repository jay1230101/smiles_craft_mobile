// Country list with ISO 3166-1 alpha-2 codes and ITU dial codes. Flag
// emojis are derived from the ISO code via regional-indicator code points
// so we don't ship 240 flag images.
...
const REGIONAL_INDICATOR_OFFSET = 127397; // 0x1F1A5 - 'A'.charCodeAt(0) = 127397
...
export const DEFAULT_COUNTRY: Country =
  COUNTRIES.find((c) => c.iso === 'LB') ?? COUNTRIES[0];

export function findCountryByIso(iso: string | null | undefined): Country | null {
  if (!iso) return null;
  const upper = iso.toUpperCase();
  return COUNTRIES.find((c) => c.iso === upper) ?? null;
}

// Longest-prefix match — pick the country whose dial code is the longest
// prefix of the given digits-only phone string. Handles overlapping codes
// like "1" vs "1242" (Bahamas) by picking the more specific match first.
export function inferCountryFromPhone(phone: string): Country | null {
...
// Strip the dial code from a stored phone string so we can show the local
// number in the input field. If no country matches, return the digits as-is.
